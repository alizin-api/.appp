...
loghandler = {
    notparam: {
        ...
        CRIADOR: `${creator}`,
        code: 406,
        MENSAGEM: 'Insira os parâmetros de apikey',
        getApikey: 'APIKEY??? Fale comigo no WhatsApp'
    },
    notkey: {
        status: false,
        CRIADOR: `${creator}`,
        code: 406,
        MENSAGEM: 'Insira o parâmetro key'
    },
    noturl: {
        status: false,
        CRIADOR: `${creator}`,
        code: 406,
        MENSAGEM: 'Insira o parâmetro url'
    },
    notkata: {
        status: false,
        CRIADOR: `${creator}`,
        code: 406,
        MENSAGEM: 'Insira o parâmetro kata'
    },
    nottext: {
        status: false,
        CRIADOR: `${creator}`,
        code: 406,
        MENSAGEM: 'Insira o parâmetro text'
    },
    nottext2: {
        status: false,
        CRIADOR: `${creator}`,
        code: 406,
        MENSAGEM: 'Insira o parâmetro text2'
    },
    notnabi: {
        status: false,
        CRIADOR: `${creator}`,
        code: 406,
        MENSAGEM: 'Insira o parâmetro nabi'
    },
    nottext3: {
        status: false,
        CRIADOR: `${creator}`,
        code: 406,
        MENSAGEM: 'Insira o parâmetro text3'
    },
    nottheme: {
        status: false,
        CRIADOR: `${creator}`,
        code: 406,
        MENSAGEM: 'Insira o parâmetro theme'
    },
    notusername: {
        status: false,
        CRIADOR: `${creator}`,
        code: 406,
        MENSAGEM: 'Insira o parâmetro username'
    },
    notvalue: {
        status: false,
        CRIADOR: `${creator}`,
        code: 406,
        MENSAGEM: 'Insira o parâmetro value'
    },
    notheme: {
    	status: false,
        CRIADOR: `${creator}`,
        code: 406,
        MENSAGEM: 'tema não está disponível, por favor entre em texmaker/list ou leia a documentação'
     },
    invalidKey: {
        status: false,
        CRIADOR: `${creator}`,
        code: 406,
        MENSAGEM: 'Apikey??? fale cmg pelo WhatsApp'
    },
    invalidlink: {
        status: false,
        CRIADOR: `${creator}`,
        MENSAGEM: 'error, talvez seu link não seja válido.'
    },
    invalidkata: {
        status: false,
        CRIADOR: `${creator}`,
        MENSAGEM: 'error, talvez a palavra não exista na api.'
    },
    notAddApiKey: {
        status: false,
        CRIADOR: `${creator}`,
        code: 406,
        MENSAGEM: 'Insira o parâmetro status, apikeyInput, email, número de celular, nome, idade, cidade, exp'
    },
    error: {
        status: false,
        CRIADOR: `${creator}`,
...
const listkey = ["DAIXSU", "DhaffXdTuruTm"];
...
router.get('/cekapikey', async (req, res, next) => {
	...
	json = JSON.stringify({
		...
		MENSAGEM: `não se esqueça de seguir ${creator}, jae?? KSKSKS`
	})
} else {
	json = JSON.stringify({
		status: false
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'alizin') return res.json(loghandler.invalidKey)
    ...
    try {
        ...
        .then(() => {
              res.json({
                  status: true,
                  CRIADOR: `${creator}`,
                  result: 'dados adicionados com sucesso, status : ' + status + ', apikey : ' + apikeyInput + ', email : ' + email + ', número do cell : ' + nomorhp + ', nome :  ' + name + ', idade : ' + age + ', cidade : ' + country + ', exp : ' + exp
              })
        })
    } catch (e) {
        console.log(e)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'alizin')  return res.json(loghandler.invalidKey)
       fetch(encodeURI(`https://raw.githubusercontent.com/DhaffXdofc/Asupan/main/cecan/vietnam.json`))
...
router.get('/cewe/thailand', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'alizin')  return res.json(loghandler.invalidKey)
       fetch(encodeURI(`https://raw.githubusercontent.com/DhaffXdofc/Asupan/main/cecan/thailand.json`))
        .then(response => response.json())
        .then(data => {
        var result = data;
        var result = data[Math.floor(Math.random() * data.length)];
...
router.get('/cewe/china', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'alizin')  return res.json(loghandler.invalidKey)
       fetch(encodeURI(`https://raw.githubusercontent.com/DhaffXdofc/Asupan/main/cecan/china.json`))
        .then(response => response.json())
        .then(data => {
        var result = data;
        var result = data[Math.floor(Math.random() * data.length)];
...
router.get('/cewe/indonesia', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'alizin')  return res.json(loghandler.invalidKey)
       fetch(encodeURI(`https://raw.githubusercontent.com/DhaffXdofc/Asupan/main/cecan/indonesia.json`))
        .then(response => response.json())
        .then(data => {
        var result = data;
        var result = data[Math.floor(Math.random() * data.length)];
...
router.get('/cewe/korea', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'alizin')  return res.json(loghandler.invalidKey)
       fetch(encodeURI(`https://raw.githubusercontent.com/DhaffXdofc/Asupan/main/cecan/korea.json`))
        .then(response => response.json())
        .then(data => {
        var result = data;
        var result = data[Math.floor(Math.random() * data.length)];
...
router.get('/cewe/japan', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'alizin')  return res.json(loghandler.invalidKey)
       fetch(encodeURI(`https://raw.githubusercontent.com/DhaffXdofc/Asupan/main/cecan/japan.json`))
        .then(response => response.json())
        .then(data => {
        var result = data;
        var result = data[Math.floor(Math.random() * data.length)];
...
router.get('/cewe/malaysia', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'alizin')  return res.json(loghandler.invalidKey)
       fetch(encodeURI(`https://raw.githubusercontent.com/DhaffXdofc/Asupan/main/cecan/malaysia.json`))
        .then(response => response.json())
        .then(data => {
        var result = data;
        var result = data[Math.floor(Math.random() * data.length)];
...
router.get('/base', async (req, res, next) => {
	var type = req.query.type,
		...
		if (!apikeyInput) return res.json(loghandler.notparam)
		if (apikeyInput != 'alizin') return res.json(loghandler.invalidKey)
		if (!type) return res.json({status: false, creator, code: 404, MENSAGEM: 'Insira o parâmetro type, type yang tersedia : base64 , base32'})
...
router.get('/covidworld', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != CRIADOR:) return res.json(loghandler.invalidKey)
...
router.get('/infocuaca', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'alizin') return res.json(loghandler.invalidKey)
	if(!provinsi) return res.json({ status : false, creator : `${creator}`, message : "Insira o parâmetro provinsi"})
...
router.get('/news/cnn', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'alizin') return res.json(loghandler.invalidKey)
    if (!type) return res.json({ status : false, creator : `${creator}`, message : "Insira o parâmetro type"})
...
router.get('/filmapik/search', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'alizin') return res.json(loghandler.invalidKey)
    if (!film) return res.json({ status : false, creator : `${creator}`, message : "Insira o parâmetro film"})
...
router.get('/filmapik/play', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'alizin') return res.json(loghandler.invalidKey)
    if (!id) return res.json({ status : false, creator : `${creator}`, message : "Insira o parâmetro id"})
...
router.get('/lk21/year', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'alizin') return res.json(loghandler.invalidKey)
    if (!tahun) return res.json({ status : false, creator : `${creator}`, message : "Insira o parâmetro tahun"})
...
router.get('/lk21/country', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'alizin') return res.json(loghandler.invalidKey)
    if (!negara) return res.json({ status : false, creator : `${creator}`, message : "Insira o parâmetro negara"})
...
router.get('/lk21/genre', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'alizin') return res.json(loghandler.invalidKey)
    if (!tipe) return res.json({ status : false, creator : `${creator}`, message : "Insira o parâmetro tipe"})
...
router.get('/yutub/search', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'alizin') return res.json(loghandler.invalidKey)
    if (!video) return res.json({ status : false, creator : `${creator}`, message : "Insira o parâmetro video"})
